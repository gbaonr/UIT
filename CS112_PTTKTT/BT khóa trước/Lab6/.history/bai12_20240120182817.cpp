...
{
    int temp = left;
    int p = a[(left + right) >> 1];

    swap(a[(left + right) >> 1], a[right]);

    // chon phan tu mid lam pivot va
    // dua cac phan tu nho hon mid ve ben trai
    // cac phan tu lon hon mid ve ben phai
    for (int i = left, p = a[(left + right) >> 1]; i <= right; i++)
        if (a[i] <= p)
            swap(a[temp++], a[i]);
...
int search(int k, int l, int r)
{
    ...
    // pivot la phan tu chia mang thanh 2 phan
