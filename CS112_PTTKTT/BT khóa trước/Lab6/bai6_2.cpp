...
int main()
{
    ...
    // khoi tao vector dp, co kich thuoc n + 1
    // dp[n] = 0 (khong co du an nao)
    vector<long long> dp(n + 1, 0);

    for (int i = n - 1; i >= 0; --i) {
        if (nextProject[i] == -1) {
            // neu khong co project tiep theo, get<2>(a[i]) la so tin chi cua do an thu i
            dp[i] = max(dp[i + 1], 1LL * get<2>(a[i]));
        } else {
            // neu co project tiep theo, get<2>(a[i]) la so tin chi cua do an thu i
            dp[i] = max(dp[i + 1], dp[nextProject[i]] + get<2>(a[i]));
        }
    }

    // xuat ra ket qua la so tin chi lon nhat co the lay duoc
    // sau khi chon cac do an tu n - 1 -> 0
    cout << dp[0] << "\n";
}