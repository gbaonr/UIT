...
void find(short index, vector<vector<short>>& matrix, vector<vector<short>>& res, vector<vector<short>>& num)
{
    ...
    // i = 0 -> khong co bom, i = 1 => co
    {
        if (!((row > 0 && (matrix[row - 1][col] - num[row - 1][col] > 0 || num[row - 1][col] > matrix[row - 1][col]))
            || (col > 0 && (matrix[row][col - 1] - num[row][col - 1] > (1 - (row == n - 1)) || num[row][col - 1] > matrix[row][col - 1]))
            || (col < m - 1 && (matrix[row][col + 1] - num[row][col + 1] > (2 - (row == n - 1) - (col == m - 2)) || num[row][col + 1] > matrix[row][col + 1]))
            || (row < n - 1 && (matrix[row + 1][col] - num[row + 1][col] > (3 - (row == n - 2) - (col == m - 1) - (col == 0)) || num[row + 1][col] > matrix[row + 1][col])))) {
    ...
    }
    {
        ...
        if (col > 0 && (num[row][col - 1] + 1 > matrix[row][col - 1] || (num[row][col - 1] + 2 - (row == n - 1) < matrix[row][col - 1])))
            continue;

        if (col < m - 1 && (num[row][col + 1] + 1 > matrix[row][col + 1] || (num[row][col + 1] + 3 - (row == n - 1) - (col == m - 2) < matrix[row][col + 1])))
            continue;

        if (row < n - 1 && (num[row + 1][col] + 1 > matrix[row + 1][col] || (num[row + 1][col] + 4 - (row == n - 2) - (col == 0) - (col == m - 1) < matrix[row + 1][col])))
            continue;
