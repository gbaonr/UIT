...
vector<vector<bitset<3>>> matrix, res, num;

short n, m;

void update(short i, short j, vector<vector<bitset<3>>>& t, short d, bool inc = true)
{
    if (i - 1 >= 0) {
        if (inc)
            t[i - 1][j] += d;
...
int main()
{
    ...
    matrix.assign(n, vector<bitset<3>>(m));
    res.assign(n, vector<bitset<3>>(m, -1));
    num.assign(n, vector<bitset<3>>(m));
