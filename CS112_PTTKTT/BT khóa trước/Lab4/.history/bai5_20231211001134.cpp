...
void find(short index, vector<vector<short>>& matrix, vector<vector<short>>& res, vector<vector<short>>& num, short n, short m)
{
    ...
    for (short i = 0; i <= 1; ++i) {
        if (i == 0) {
            ...
            if ((matrix[row][col - 1] - num[row][col - 1] > (1 - (row == n - 1))
                    || num[row][col - 1] > matrix[row][col - 1])) {
                continue;
            }

            if ((matrix[row][col + 1] - num[row][col + 1] > (2 - (row == n - 1) - (col == m - 2))
                    || num[row][col + 1] > matrix[row][col + 1])) {
                continue;
            }

            if ((matrix[row + 1][col] - num[row + 1][col] > (3 - (row + 1 == n - 1) - (col == m - 1) - (col == 1))
                    || num[row + 1][col] > matrix[row + 1][col])) {
                continue;
            }
