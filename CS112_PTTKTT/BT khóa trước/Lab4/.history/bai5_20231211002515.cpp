...
void find(short index, vector<vector<short>>& matrix, vector<vector<short>>& res, vector<vector<short>>& num)
{
    ...
    for (short i = 0; i <= 1; ++i) {
        if (i == 0) {
            ...
            if (row > 0 && (num[row - 1][col] + 1 > matrix[row - 1][col] || (num[row - 1][col] < matrix[row - 1][col])))
                continue;

            if (col > 0 && (num[row][col - 1] + 1 > matrix[row][col - 1]))
                continue;

            if (col < m - 1 && (num[row][col + 1] + 1 > matrix[row][col + 1]))
                continue;

            if (row < n - 1 && (num[row + 1][col] + 1 > matrix[row + 1][col]))
                continue;
