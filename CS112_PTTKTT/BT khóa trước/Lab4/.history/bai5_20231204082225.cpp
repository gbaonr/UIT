...
void update(short i, char j, vector<vector<char>>& t, char d)
{
    if (i - 1 >= 0)
        t[i - 1][j] += d;
...
void update2(short i, char j, vector<vector<char>>& t, char d)
{
    if (i - 1 >= 0)
        t[i - 1][j] = d;
...
bool checkConflict(short i, char j)
{
...
int main()
{
    ...
    matrix.assign(n, vector<short>(m));
    res.assign(n, vector<short>(m, -1));
    num.assign(n, vector<short>(m));
