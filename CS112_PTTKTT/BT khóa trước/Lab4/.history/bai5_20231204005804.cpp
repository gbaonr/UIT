...
void update(int i, int j, vector<vector<int>>& t, int d, bool inc = true, bool checkRes = true)
{
    for (auto v : { -1, 1 }) {
        if (i + v >= 0 && i + v < n) {
            if (checkRes && res[i + v][j] == -1) {
                if (inc)
                    t[i + v][j] += d;
                else
                    t[i + v][j] = d;
