...
{
    return (num[i - 1][j] > matrix[i - 1][j])
        || (num[i + 1][j] > matrix[i + 1][j])
        || (num[i][j - 1] > matrix[i][j - 1])
        || (num[i][j + 1] > matrix[i][j + 1]);
}
...
void find(short index, vector<vector<short>>& matrix, vector<vector<short>>& res, vector<vector<short>>& num, short n, short m)
{
    ...
    for (short i = 0; i <= 1; ++i) {
        if (i == 0) {
            ...
            if ((matrix[row][col - 1] - num[row][col - 1] > 1 || num[row][col - 1] > matrix[row][col - 1])) {
                continue;
            }

            if ((matrix[row][col + 1] - num[row][col + 1] > 2 || num[row][col + 1] > matrix[row][col + 1])) {
                continue;
            }

            if ((matrix[row + 1][col] - num[row + 1][col] > 3 || num[row + 1][col] > matrix[row + 1][col])) {
                continue;
            }
...
int main()
{
    ...
    vector<vector<short>> matrix(n + 2, vector<short>(m + 2, 1000));
    vector<vector<short>> res(n + 2, vector<short>(m, -1));
    vector<vector<short>> num(n + 2, vector<short>(m + 2, 0));
