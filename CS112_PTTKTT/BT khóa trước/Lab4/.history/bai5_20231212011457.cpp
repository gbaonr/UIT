    ...
    for (int t = 0; t < 2; ++t) {
        for (short i = 0; i < n; ++i) {
            for (short j = 0; j < m; j++) {
                ...
                totalBoom += (i - 1 >= 0 && res[i - 1][j] == 1);
                totalBoom += (i + 1 < n && res[i + 1][j] == 1);
                totalBoom += (j - 1 >= 0 && res[i][j - 1] == 1);
                totalBoom += (j + 1 < m && res[i][j + 1] == 1);

                totalMiss += (i - 1 >= 0 && res[i - 1][j] == -1);
                totalMiss += (i + 1 < n && res[i + 1][j] == -1);
                totalMiss += (j - 1 >= 0 && res[i][j - 1] == -1);
                totalMiss += (j + 1 < m && res[i][j + 1] == -1);
