...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        """
        print("evaluationFunction is here")
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def minValue(state, agentIdx, depth, alpha, beta):
            ...
            value = INF # define value = +infinity
            for action in state.getLegalActions(agentIdx):
                succ = minValue(state.generateSuccessor(agentIdx, action), agentIdx + 1, depth, alpha, beta)
                # print(f"        succ: {type(succ)}, value: {type(value)}, agent = {agentIdx+1} ")
                value = min(value, succ)

                # prunning using alpha, beta
                if value <= alpha:
                    return value
                beta = min(beta, value)
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def expectimax(state):
            ...
            for action in state.getLegalActions(0):
                succ = exp(state.generateSuccessor(0, action), 1, 1)
                value.append(succ)
                if bestValue is None:
                    bestValue = succ
                    bestAction = action
        ...
        def exp(state, agentIdx, depth):
            if agentIdx == state.getNumAgents():
                return maxValue(state, 0, depth+1)
            value = 0

            num_actions = len(state.getLegalActions(agentIdx))

            for action in state.getLegalActions(agentIdx):
                succ = exp(state.generateSuccessor(agentIdx, action), agentIdx+1, depth)

                value += succ/num_actions
...
def myEvalFunc(currentGameState):
    """
    ...
    foodDistance.sort()
    while(len(foodDistance) < 2):
        foodDistance.append(0)
        
    # calculate state score
    score = -1.5*foodDistance[0] + ghost_dis + cap_dis - 10*len(foodList)
    score += -10*len(newCapsules)

    score += scoreEvaluationFunction(currentGameState)
    score += currentGameState.getScore()*2

    #try to eat capsule -> hunt ghost
    # if the ghosts are scared
    if sum(newScaredTimes) > 0:
        score += 10*len(newCapsules)
        
        for index,ghost in enumerate(newGhostStates):
            curr_dis = manhattanDistance(newPos, ghost.getPosition())

            if curr_dis <= closestGhost and ghost.scaredTimer > 0 :
                score += 30/curr_dis
    else:
        score += -0.5*foodDistance[1]
