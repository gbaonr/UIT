...
def select_level():
    ...
    screen2 = pygame.display.set_mode([400, 50])  
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_BACKSPACE:
                    user_txt = user_txt[0:-1]
                elif event.key == K_RETURN:
                    user_txt = int(user_txt)
                    print("selected level: " + str(user_txt))
                    return user_txt
                    pygame.quit()
                    sys.exit()
                else:
...
class Menu:
    ...
    def click(self, click_pos, window, level = 1):
        x = click_pos[0]
        y = click_pos[1]
        sokoban = Game(window, level)
...
def main():
    ...
    while run:
        ...
        if event.type == MOUSEBUTTONUP:
            run = menu.click(event.pos, window, user_level)

        pygame.draw.rect(window, SOKOBAN.BLACK, (0,0,SOKOBAN.WINDOW_WIDTH,SOKOBAN.WINDOW_HEIGHT))
        menu.render(window)
        pygame.display.flip()
